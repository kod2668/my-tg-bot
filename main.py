...
# --- TELEGRAM COMMANDS (Command သုံးရန်နှင့် ညွှန်ကြားရန် သက်သက်) ---
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if user_id == OWNER_ID:
        await update.message.reply_text("👑 မင်္ဂလာပါ Boss။\n\nApp Users များကို VIP ပေးရန်:\n`/addvip [Device_ID]` ဟု အသုံးပြုပါ။\n\nVIP ပြန်ဖြုတ်ရန်:\n`/removevip [Device_ID]` ဟု အသုံးပြုပါ။")
    else:
        await update.message.reply_text("⛔ ဤ Bot ကို ပိုင်ရှင် (Owner) သာ အသုံးပြုနိုင်ပါသည်။")
...
async def add_vip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        conn.commit()
        await update.message.reply_text(f"⭐ အောင်မြင်ပါပြီ Boss!\nDevice ID: `{target_device_id}` ကို VIP သတ်မှတ်ပြီးပါပြီ။")
...
# App UI ကနေ စာနဲ့ Device ID လှမ်းပို့တာကို လက်ခံမည့် /chat route
...
def web_chat():
    try:
        ...
        # ဒီ Device ID က VIP ဟုတ်မဟုတ် စစ်ဆေးမည်
...
def main():
    # Start Flask thread for Render web service
    web_thread = Thread(target=run_web)
    web_thread.daemon = True
    web_thread.start()

    # Telegram Bot Setup (Command တွေအတွက် သက်သက်)
