...
# ==================== VIP & ADMIN SQLITE DATABASE ====================
# Keep the existing SQLite database for VIP/Admin so the current
# Telegram bot functionality is not changed.
def init_db():
...
def api_remove_vip():
    ...
    try:
        cursor.execute(
            "DELETE FROM vips WHERE device_id = ?",
            (device_id,)
        )
        conn.commit()
        return jsonify({"success": True})
...
def api_user_stats():
    ...
    try:
        ...
        # VIP status is stored in the existing SQLite database.
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        cursor.execute("SELECT device_id FROM vips")
        vip_devices = {row[0] for row in cursor.fetchall()}
        conn.close()
