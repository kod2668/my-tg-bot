...
def get_conversation_history(conversation_id, user_id, device_id):
    """Load the current conversation from PostgreSQL for Gemini context."""
    with get_pg_connection() as conn:
        with conn.cursor() as cur:
            if not conversation_belongs_to_user(
                cur, conversation_id, user_id, device_id
...
def api_chat():
    ...
    # ==================== LOAD POSTGRES HISTORY AS GEMINI CONTEXT ====================
    try:
        history_rows = get_conversation_history(
            conversation_id,
            user_id_int,
            device_id
        )
        ...
        return Response(
            f"[Error loading conversation context: {str(e)}]",
            mimetype='text/plain',
            status=500
        )

    # Gemini expects the conversation in role-based Content objects.
    # The current user message was already saved to PostgreSQL above,
    # so the full history already contains the current prompt.
    contents = []
    ...
    # If an image/file was uploaded with the current user message,
    # attach it to the latest user turn without changing the database schema.
    if file_data:
        if contents and contents[-1].role == "user":
            contents[-1].parts.append(file_data)
        else:
            contents.append(
                types.Content(
                    role="user",
