...
# ==================== POSTGRES DATABASE ====================
# VIP, Admin, and chat-history tables are stored in Railway PostgreSQL.
# sqlite3 is retained only for one-time migration of legacy data.
    ...
    try:
        with get_pg_connection() as conn:
            with conn.cursor() as cur:
                ...
                # ==================== POSTGRES VIP DATABASE ====================
                # VIP records are stored in the same Railway PostgreSQL
                # database as chat history, so they persist across deploys.
                cur.execute("""
                ...
                cur.execute("""
                    CREATE INDEX IF NOT EXISTS idx_vips_expiry
                    ON vips (expiry_date)
                """)
        ...
        migrate_legacy_sqlite_data_to_postgres()
        print("PostgreSQL chat history + VIP + Admin database initialized.")
    except Exception as e:
        print(f"WARNING: PostgreSQL initialization failed: {e}")


def migrate_legacy_sqlite_data_to_postgres():
    """One-time migration from the old SQLite VIP/Admin tables.

    SQLite is used only as a legacy source. All active application data is
    stored in PostgreSQL after migration. Existing PostgreSQL records are
    preserved.
    """
    if not DATABASE_URL or not os.path.exists(DB_NAME):
        return
    ...
    try:
        ...
        try:
            sqlite_cursor.execute("SELECT device_id, expiry_date FROM vips")
            legacy_vips = sqlite_cursor.fetchall()
        except sqlite3.Error:
            pass

        try:
            sqlite_cursor.execute("SELECT admin_id FROM admins")
            legacy_admins = sqlite_cursor.fetchall()
        except sqlite3.Error:
            pass

        sqlite_conn.close()
    except Exception as e:
        print(f"WARNING: Could not read legacy SQLite data: {e}")
        return
    ...
    try:
        with get_pg_connection() as conn:
            with conn.cursor() as cur:
                migrated_vips = 0
                for device_id, expiry_str in legacy_vips:
                    if not device_id or not expiry_str:
                        continue
                    try:
                        ...
                        print(
                            f"WARNING: Skipping invalid legacy VIP expiry for {device_id}: "
                            f"{expiry_str}"
                        )
                        continue
                    ...
                    cur.execute("""
                        ...
                        SET expiry_date = GREATEST(vips.expiry_date, EXCLUDED.expiry_date)
                    """, (device_id, expiry_dt))
                    migrated_vips += 1

                migrated_admins = 0
                for (admin_id,) in legacy_admins:
                    if not admin_id:
                        continue
                    cur.execute("""
        ...
        if migrated_vips or migrated_admins:
            print(
                f"Migrated legacy SQLite data: {migrated_vips} VIP(s), "
                f"{migrated_admins} admin(s)."
            )
    except Exception as e:
        print(f"WARNING: PostgreSQL legacy data migration failed: {e}")
...
def is_admin(user_id: int) -> bool:
    ...
    try:
        ...
        print(f"WARNING: Admin lookup failed: {e}")
        return False
    ...
    with get_pg_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT 1
                FROM vips
                WHERE device_id = %s AND expiry_date >= NOW()
            """, (device_id,))
            res = cur.fetchone()
...
def clean_expired_vips():
    ...
    with get_pg_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("DELETE FROM vips WHERE expiry_date < NOW()")
        conn.commit()
...
def api_add_vip():
    ...
    expiry_date = datetime.now(timezone.utc) + timedelta(days=int(days))
...
def api_user_stats():
    """Return total, free, and active VIP users based on chat history/device IDs."""
    clean_expired_vips()

    try:
        # PostgreSQL contains the users/devices that have actually used chat.
        with get_pg_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("""
        ...
        # VIP status is stored in PostgreSQL.
        with get_pg_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT device_id FROM vips WHERE expiry_date >= NOW()")
                vip_devices = {row[0] for row in cur.fetchall()}

        total_users = len(user_rows)
        vip_users = sum(1 for _, device_id in user_rows if device_id in vip_devices)
        ...
        return jsonify({"error": f"Could not load user statistics: {str(e)}"}), 500
